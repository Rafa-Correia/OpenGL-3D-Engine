...
camera::camera(vec3<float> pos, vec3<float> target_lock_point, vec3<float> up, std::vector<vec3<float>> l_pos)
{
    this->pos = pos;
    this->up = up;
    ...
    if (l_pos.size() == 0)
    {
        lock_point_list.push_back(vec3<float>());
    }
...
void camera::update_camera_position(const bool key_states[256], int delta_time_ms)
{
    ...
    if (is_free_camera)
    {
        ...
        {
            vec3<float> cross = vec3<float>::cross(up, dir);
            cross.y = 0;
            cross.normalize();
...
void camera::update_lock_positions(std::vector<vec3<float>> &l_pos)
{
...
mat4 camera::get_view_matrix()
{
    ...
    return mat4::View(pos, lock_point, up);
}
