...
public:
    camera();
    camera(vec3<float> pos, vec3<float> lock_point, vec3<float> up, std::vector<vec3<float>> l_pos = std::vector<vec3<float>>());
     ...
     */
    void update_lock_positions(std::vector<vec3<float>> &l_pos);

private:
    vec3<float> pos; // < -- position of the camera
    vec3<float> up;  // < -- camera's "up" vector (vector pointing up)
    vec3<float> dir; // < -- camera direction, used in free cam mode

    vec3<float> lock_point; // < -- camera always looks at this point in fixed mode

    vec3<float> target_lock_point; // < -- camera's lock point (in fixed mode, after animation, target point will be equal to this)
    vec3<float> start_lock_point;  // < -- when playing lerp camera transition, this serves as the start point for camera target

    vec3<float> target_pos; // < -- camera's target position (will move to this point during change target animation)
    vec3<float> start_pos;  // < -- serves as starting posititon in lerp transition of movement
    ...
    std::vector<vec3<float>> lock_point_list; // < -- list of points for camera locking.
